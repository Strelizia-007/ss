...
async def main():
    ...
    # File-based session — persists auth between restarts
    ...
    # Pre-export auth to all DCs so borrowed senders work instantly (no stall)
    logger.info("Pre-exporting auth to all DCs...")
    try:
        ...
        for dc_id in [1, 2, 3, 4, 5]:
            try:
                await tele._get_dc(dc_id)  # ensure DC info is loaded
                sender = await tele._borrow_exported_sender(dc_id)
                await tele._return_exported_sender(sender)
                logger.info(f"  DC{dc_id} auth ready ✅")
            except Exception as e:
                logger.warning(f"  DC{dc_id} pre-auth skipped: {e}")
    except Exception as e:
        logger.warning(f"DC pre-auth failed (non-fatal): {e}")
