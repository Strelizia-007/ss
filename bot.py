...
async def process_thumb(uid: int, chat_id: int):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 1. Try Telegram's own document thumbnail — instant, no download
        if tele_doc and getattr(tele_doc, "thumbs", None):
            try:
                result = await tele.download_media(tele_doc, file=out,
                    thumb=-1)  # -1 = largest available thumb
                if result and os.path.exists(out) and os.path.getsize(out) > 0:
                    ...
                    sent = True
                    logger.info("Thumbnail: used Telegram document thumb")
            except Exception as e:
                logger.warning(f"Telegram thumb failed: {e}")

        # 2. Fallback: frame grab via ffmpeg from stream URL
        if not sent:
            path = await get_input_source(uid, tmpdir)
            if not path:
                await prog.edit_text("❌ Could not get file."); return
